...
最終目的は自動化による利益獲得です。Phase1 では実注文を行わないシミュレーション基盤として作り、2026-08-30 から Phase3（実注文と安全制御）として実資金で稼働しています。安全は手動の承認ではなく、「金額の上限」と「システムが自分で止まること」で担保します。
...
- **注意 (実資金が動きます):** 2026-08-30 から Phase3 として本番で実注文を行っています。設定を変えると実際のお金が動きます。
  - 実発注されるのは、`app.phase` が 3 以上、`real_trade_enabled` が `true`、`dry_run` が `false` の**3つがそろったときだけ**です。1つでも欠ければ発注しません。GitHub Variables に未登録の場合は、安全側の値（Phase1・無効・dry-run）が使われます。
  - 上限と自動停止（1回・1日の注文額、保有額、日次損失、連敗）は [config/application-gmo.yaml](config/application-gmo.yaml) と GitHub Variables を正とします。
  - 稼働状況の確認方法は [デプロイ状態の確認 (docs/operations/gcp/09-status-check.md)](docs/operations/gcp/09-status-check.md) にあります。
  - 残金と保有状況を `state.json` に保存します。
