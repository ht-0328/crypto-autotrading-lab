...
| 項目 | 内容 |
| --- | --- |
| 想定読者 | 次にどの作業へ着手するかを決める人 |
| 読んだあとできること | 実注文に到達するまでの順序と、実資金を入れてよい条件を説明できる |
| 状態 | 現行 |
| 機密区分 | 公開可 |
| 作成者 | リポジトリ管理者 |
| 保守責任者 | リポジトリ管理者 |
...
3ツールで現状を評価し、結果を突き合わせて作成しています（2026-08-29）。[改善計画](../improvements/README.md) の第1波・第2波（PR01〜PR10）が完了した時点の状態が出発点です。

3ツールの結論は一致しています。**現在のコードのまま実注文を有効にしてはいけません。**

## 最重要: いま実注文を有効にすると何が起きるか

!!! danger "現在のコードのまま実注文を有効にしてはいけません"

    [RealTradingService](https://github.com/ht-0328/crypto-autotrading-lab/blob/main/projects/crypto-autotrading-app/src/main/kotlin/cryptoautotrading/domain/realtrading/RealTradingService.kt) は買い注文だけを送ります。**売り注文は送りません。** `SELL_CANDIDATE` はログを出すだけです。
    ...
    `max_order_jpy` などの上限は「1回に賭ける金額」を制限するだけです。**すでに持っているポジションの下落は止めません。**これは「自動売買」ではなく「出口のないポジションを作る処理」です。
...
- **PLAN00 が先**: [ロードマップ](../overview/roadmap.md) の記述が実態と噛み合っていません。
  Phase3 の禁止事項に「手動の承認なしの注文」があり、着手条件も Phase2 の完了になっています。この矛盾を先に解きます。そのあとで以降の計画の完了条件を決めます。
- **PLAN01 が2番目**: 売れない状態では、安全ガードを積んでも下落は止まりません。ここが唯一「これが無いと絶対にダメ」な項目です。
- **PLAN02 が3番目**: 売買サイクルが閉じてから、誤発注・二重発注の経路を潰します。
- **PLAN03 が4番目**: 実資金を入れる前に2つを用意します。異常を人間が知る手段と、システムが自分で止まる条件です。**手動承認を置かない方針（下記）なので、ここが人間側の唯一の歯止めです。**
...
[ロードマップ](../overview/roadmap.md) は PLAN00 で書き換え済みです。Phase2 を Phase2a（通知）と Phase2b（Web画面）に分けています。Phase3 の着手条件は Phase2a の完了です。Phase3 の禁止事項から「手動の承認なしの注文」を外しています。代わりに安全ルールを置いています。

承認を置かない代わりに、次の3つが**必須**です。どれも「後で入れる」にはできません。

1. 口座に入れる金額の上限（30,000円）
2. 自動で止まる条件 — **まだ1つも実装されていません**
   - 1日の損失上限 2,000円、3連敗、スリッページ 0.5%
3. 通知と緊急停止
...
3. **安全チェックが買い専用**（[PLAN01](plan01-real-sell-order.md)）— 「保有中なら注文しない」という条件です。そのまま売りに使うと逆の挙動になります。

**解消済み**

4. ~~**数量の刻み（`sizeStep`）に丸めていない**~~ — 解消済み（[PLAN02](plan02-order-safety-guards.md) の A）。GMOコインの BTC は最小注文数量・刻みとも 0.00001 です。
5. ~~**注文数量の計算にK線の終値を使っている**~~ — 解消済み（[PLAN02](plan02-order-safety-guards.md) の B）。Ticker の最新価格を使い、手数料も上限判定に含めるようにしています。

**未解消（続き）**

6. **発注の「意図」を送信前に保存していない**（[PLAN01](plan01-real-sell-order.md)）— POST 後に落ちると記録が残りません。

**解消済み（続き）**

7. ~~**通知が1つも実装されていない**~~ — 解消済み（[PLAN03](plan03-notification.md)）。日次サマリーと沈黙の検知は積み残しです。
8. ~~**損失上限・連敗停止・スリッページ上限が未実装**~~ — 解消済み（[PLAN02](plan02-order-safety-guards.md)）。

**未解消（口座）**

9. **同一口座の既存資産を巻き込む危険**（[PLAN01](plan01-real-sell-order.md)）— 全量売ると、他が買った BTC まで売ります。専用口座を推奨します。
