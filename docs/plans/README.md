...
- **PLAN00 が先**: [ロードマップ](../overview/roadmap.md) の記述が実態と噛み合っていません。
  Phase3 の禁止事項に「手動の承認なしの注文」があり、着手条件も Phase2 の完了になっています。この矛盾を先に解いてから、以降の計画の完了条件を決めます。
...
6. **発注の「意図」を送信前に保存していない**（[PLAN01](plan01-real-sell-order.md)）— POST 後に落ちると、アプリ側に記録が残りません。
...
9. **同一口座の既存資産を巻き込む危険**（[PLAN01](plan01-real-sell-order.md)）— 全量売ると、ボット以外が買った BTC まで売ります。専用口座を推奨します。
