...
- **PLAN00 が先**: 現在の [ロードマップ](../overview/roadmap.md) は Phase3 の禁止事項として「手動の承認なしで実際の注文を出すこと」を挙げ、着手条件として Phase2（Web画面を含む）の完了を求めています。この矛盾を先に解いてから、以降の計画の完了条件を決めます。
- **PLAN01 が2番目**: 売れない状態で安全ガードを積んでも、ポジションの下落は止まりません。ここが唯一「これが無いと絶対にダメ」な項目です。
...
[ロードマップ](../overview/roadmap.md) は PLAN00 で書き換え済みです。Phase2 を Phase2a（通知）と Phase2b（Web画面）に分けています。Phase3 の着手条件は Phase2a の完了です。Phase3 の禁止事項から「手動の承認なしで実際の注文を出すこと」を外し、代わりに安全ルールを置いています。
