...
4. **数量の刻み（`sizeStep`）に丸めていない**（[PLAN02](plan02-order-safety-guards.md)）— GMOコインの BTC は最小注文数量・刻みとも 0.00001 です。現在のコードは小数8桁で切り捨てるだけなので、刻みの倍数にならず取引所に拒否されます。金額を上げても解決しません。
