...
現在の [ロードマップ](../overview/roadmap.md) は Phase3 の禁止事項に「手動の承認なしで実際の注文を出すこと」を挙げています。**この決定は文書と矛盾するので、[PLAN00](plan00-phase-and-safety-contract.md) で正式に書き換えてから進んでください。** 黙って破らないこと。

承認を置かない代わりに、次の3つが**必須**になります。どれも「後で入れる」にはできません。

1. 口座に入れる金額の上限（全額失っても困らない額）
2. 自動で止まる条件（1日の損失上限、連敗停止、スリッページ上限）
3. 通知と緊急停止
...
4. **最小注文数量・数量刻みに対応していない**（[PLAN02](plan02-order-safety-guards.md)）— 現在の `trade_amount: 1000` / `max_order_jpy: 1000` では、BTC の価格次第で最小注文数量を下回り、毎回拒否されます。
