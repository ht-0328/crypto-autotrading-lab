...
- **PLAN00 が先**: [ロードマップ](../overview/roadmap.md) の記述が実態と噛み合っていません。
  Phase3 の禁止事項に「手動の承認なしで実際の注文を出すこと」があり、着手条件も Phase2 の完了になっています。この矛盾を先に解いてから、以降の計画の完了条件を決めます。
...
2. 自動で止まる条件 — **まだ1つも実装されていません**（1日の損失上限 2,000円、3連敗、スリッページ 0.5%）
3. 通知と緊急停止
...
6. **発注の「意図」を送信前に保存していない**（[PLAN01](plan01-real-sell-order.md)）— POST 後・保存前に落ちると、アプリ側に記録が残りません。
...
9. **同一口座の既存資産を巻き込む危険**（[PLAN01](plan01-real-sell-order.md)）— 残高をそのまま全量売ると、ボット以外が買った BTC まで売ります。専用口座を推奨します。
