...
5. 手数料（成行は 0.05%）を注文金額の上限判定に含める。上限ぎりぎりの注文で、手数料の分だけ上限を超えないようにする。**未実施。B とあわせて実施する。**
...
2. B（注文価格の基準とスリッページ）+ A の手数料考慮
3. backlog 5（入力値検証）
4. backlog 6 のうち Clock 注入
5. backlog 1（市場データ検証）
6. backlog 2（リトライとタイムアウト）
7. backlog 3（重複実行抑止）
8. backlog 8（6時境界）
