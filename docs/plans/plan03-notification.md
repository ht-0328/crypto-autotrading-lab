...
| 条件 | 挙動 |
| --- | --- |
| 1日の損失が上限に達した | その日は新規の買いを止める |
| 連敗が規定回数に達した | 新規の買いを止める |
| 約定価格が想定から規定率以上乖離した | 新規の買いを止める |
...
人間が即座に止める手段を用意します。現在は `state.json` の直接編集か Cloud Scheduler の停止しかありません（[復旧手順](../operations/real-trading-recovery.md)）。**実際に一度試して、止まることを確認してください。**
