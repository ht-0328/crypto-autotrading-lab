...
**状態**: 実施済み（沈黙の検知は [PLAN06](plan06-unattended-trading.md) で扱う）
...
| 日次サマリー | 損益、保有状況、総資産、連敗。**実施済み。1日1回** |
| heartbeat（定期実行が生きていること） | 日次サマリーが兼ねる。**アプリ自身は自分が動いていないことを通知できない**ため、沈黙の検知は外部監視で行う（[PLAN06](plan06-unattended-trading.md)） |

**heartbeat は忘れがちですが重要です。** ジョブが起動しなくなった場合、通知は「来ない」だけなので、異常が沈黙として現れます。定期的な生存通知が無いと、止まったことに気づけません。

ただし、**アプリ自身は「自分が動いていないこと」を通知できません。** 沈黙を検知するには外側からの監視が要ります。5分ごとに通知を送ると1日288件になり、本当に伝えたいことが埋もれるため、アプリからの生存通知は1日1回のサマリーにまとめる方針とします。実装は下記の積み残しです。
...
- **沈黙の検知**。アプリ自身は自分が動いていないことを通知できません。日次サマリーが「来ない」ことに気付く仕組みは外側に必要で、[PLAN06](plan06-unattended-trading.md) で扱います。
