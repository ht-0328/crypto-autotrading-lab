...
損切り後にすぐ再エントリーしない方針の仕様を決めます。
`SafeReboundStrategy` との違いと、バックテストでの比較観点も整理します。
...
`CooldownReboundStrategy` は下落後の反発を狙う点で `SafeReboundStrategy` と同じです。
これに「損切り直後の一定期間は再エントリーしない」クールダウン期間を設けます。連続下落相場での「往復ビンタ」を減らし、最大ドローダウンを抑えることが目的です。
