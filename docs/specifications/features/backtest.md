...
バックテストは、過去K線CSVから読み込んだ `List<Kline>` の上で売買戦略を動かします。
実際の注文を行う前に、戦略の成績を確認します。
...
過去のK線データを古い順に1本ずつ処理します。その時点のデータで売買判定を行います。判定結果に応じて仮想の残高・保有数量・損益を更新し、バックテスト結果をサマリーおよび明細のCSVとして出力します。

判定に使ったK線の終値でそのまま約定させると、成績が実態より良く出ます。
「終値を見てから、その終値で売買できる」ことになるためです。これを避けるため、**K線 N の確定後に出したシグナルは K線 N+1 の始値で約定させます**。
...
`BACKTEST_FEE_RATE` と `BACKTEST_SLIPPAGE_RATE` はエラーにする場合があります。
数値として解釈できない場合と、負の値の場合です（成績を誤って評価しないため）。
...
6. 直前のK線のシグナルがあれば、**このK線の始値**を約定価格として状態を更新する
7. その時点までのK線一覧を TradingStrategy に渡す
8. TradingStrategy の判定結果を受け取り、次のK線で約定させるシグナルとして保持する
9. 残高・保有数量・買値・確定損益を記録する
10. 保有評価額と総資産を、そのK線の終値で計算する
...
明細CSVの各行は2つを並べます。「そのK線で出たシグナル」と「始値で約定した結果の状態」です。したがって、ある行の売買判定が状態に反映されるのは次の行です。
