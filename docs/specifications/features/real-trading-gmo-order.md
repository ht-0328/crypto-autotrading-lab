...
!!! danger "この仕様は Phase3（実注文 + 安全制御）のスコープで、本番で有効です"

    2026-08-30 から本番でこの経路が有効になっています。手動の承認は挟まず、安全は金額の上限とシステム自身の停止で担保します（[ロードマップ](../../overview/roadmap.md) の「手動承認を置かない決定」）。

    `app.phase` が 3 未満の環境では、実注文が有効な設定を検出した時点で起動時ガードにより異常終了します。Phase1 側の扱いは [Phase1 仕様書](../phase1-simulation.md) を参照してください。
...
| 設定: max_daily_loss_jpy | 2000 | 必須 | 1日の損失の上限。達したらその日は新規の買いを止める |
| 設定: max_consecutive_losses | 3 | 必須 | 連敗の上限。達したらその日は新規の買いを止める |
...
安全条件を判定する前に、渡された値そのものが妥当かを確認します。安全境界は上流が正しい値を渡してくることを前提にしません。次のいずれかに当てはまる場合、他の条件を見るまでもなく注文しません。
...
| 保有数量が負の値 | 買い |
| 1日の累計注文額が負の値 | 買い |
| `max_order_jpy` / `max_daily_order_jpy` / `max_position_jpy` が0以下 | 買い |
| 売却数量が0以下 | 売り |
| 記録上の保有数量が0以下 | 売り |
| 取引所の売却可能残高が負の値 | 売り |
...
| 1日の累計注文額が max_daily_order_jpy 以下 | 次のチェックへ（※4） |
...
手動の承認を置かない運転では、負けが込んだときに止めるのはシステム自身です。次のいずれかに当てはまる場合、**その日は新規の買いを行いません**。

| 条件 | 設定 | 解除 |
| --- | --- | --- |
| その日の確定損益が損失の上限に達した | `max_daily_loss_jpy` | 日付が変われば自動で解除 |
| 連敗が上限に達した | `max_consecutive_losses` | 日付が変われば自動で解除 |

- **止めるのは新規の買いだけです。** 保有を解消する売りは止めません。売りまで止めると、ポジションを抱えたまま損切りできなくなります。
- **日付が変われば自動で解除されます。** 解除されないと、停止したまま売買が行われず、連敗が途切れる機会も無くなって永久に止まったままになります。
- 連敗の回数は、売りの約定で損失が確定するたびに増え、利益が確定すると0に戻ります。
- どちらも未設定のまま実注文を有効にすると、起動時に異常終了します。

### 8.6 発注意図の先行保存

注文を送ったあと、結果を保存する前にプロセスが落ちると、取引所には注文があるのにアプリ側には何の記録も残りません。次の実行は「注文していない」と判断し、もう一度注文を出しうる状態になります。**二重注文です。**

これを防ぐため、**注文を送信する直前に「これから何を注文するか」を状態に保存**します。

| 場面 | 記録される内容 |
| --- | --- |
| 送信の直前 | `orderId` は `null`、`status` は `WAITING`、売買区分・数量・想定価格・時刻 |
| 送信の成功後 | 取引所が返した `orderId`、`status` は `ORDERED` |

次の実行で `orderId` が `null` のまま `WAITING` の注文を見つけた場合、**取引所に届いたかどうかが分かりません**。その場合は新しい注文を出さず、`isStopped=true` にして人の確認を待ちます。復旧は [復旧手順](../../operations/real-trading-recovery.md) を参照してください。

**意図の保存に失敗した場合は注文を送りません。** 記録が残らないまま注文するくらいなら、注文しないほうが安全です。

### 8.7 停止中（`isStopped=true`）の振る舞い
...
!!! note "（※4）1日の累計注文額の数え直し"

    累計は**日本時間の日付**で数え直します。記録されている日付が今日と違えば、その日はまだ1件も注文していないものとして0から数えます。日付を見ずに累計をそのまま使うと、上限に近づいた翌日以降は1件も注文できなくなります。

!!! note "（※1）stop_on_unconfirmed_order について"
...
手動の承認を置かないため、発注の直前に人が内容を見る機会がありません。**起きたことを事後に必ず知ることだけが、人間側の歯止めです。**

| 出来事 | 重大度 | 内容 |
| --- | --- | --- |
| 実注文の送信 | INFO | 売買区分、銘柄、数量、想定価格、想定金額 |
| 約定の確認 | INFO | 約定価格、数量、手数料、確定損益（売りの場合） |
| 自動停止の発動（損失上限・連敗） | WARN | 停止した理由と、売りは止まらないこと |
| リアル取引の停止（`isStopped`） | CRITICAL | 停止理由と、復旧が自動では行われないこと |
| 日次サマリー（1日1回） | INFO | モード、価格、保有状況、残金、総資産、確定損益、連敗、直近の判定 |

- **送信に失敗した日次サマリーは、次の実行で再び送ります。** 送れていないのに「送った」と記録すると、その日の分が永久に失われます。サマリーが届かないことは「システムが止まった」ことの唯一の手がかりなので、通信障害の日ほど生存確認が失われるのは避けなければなりません。
- **HTTP のステータスを検証します。** 2xx 以外は送信できなかったものとして扱います。検証しないと、Webhook が削除されていても送れたことになります。
- **日次サマリーは1日1回だけ送ります。** 実行のたびに送ると5分間隔では1日288件になり、本当に伝えたいことが埋もれます。一方でまったく送らないと、ジョブが止まったことに気付けません。**通知が「来ない」ことが異常の唯一の手がかり**になるため、1日1回は必ず届く経路を作っています。
- **日常的な見送りは通知しません。** 保有中や未約定注文ありといった見送りは5分ごとに起こりうるため、通知すると本当に伝えたいことが埋もれます。
- **通知の送信に失敗しても売買処理は止めません。** 通知は観測の手段であって、それ自体が売買を止める理由にはなりません。失敗はログに残します。
- **送信先のURLは秘密情報です。** 設定ファイルには書かず、環境変数 `NOTIFICATION_WEBHOOK_URL` から渡します。通知の本文にも含めません。
- 送信先は Webhook です。Discord と Slack はどちらも「JSONの1キーに本文を入れて POST する」形式なので、`notification.payload_key` を切り替えるだけで両方に対応できます（Discord は `content`、Slack は `text`）。

## 8.9 時刻の扱い

売買の時刻はすべて**日本時間**を基準にします。GMOコインは日本の取引所で、営業日の切り替え（朝6時）も1日の累計注文額の集計も日本時間が基準になるためです。

実行環境のタイムゾーン設定に左右されないよう、基準となるタイムゾーンは `TradingTime.ZONE` に固定しています。現在時刻そのものは `java.time.Clock` として外から注入します。domain の中で直接取得すると、日付境界の挙動を固定した時刻で検証できなくなるためです。
...
!!! note "補足"
    ...
    売り注文は `realTrading.isStopped=true` でも実行します（上記 8.6）。
