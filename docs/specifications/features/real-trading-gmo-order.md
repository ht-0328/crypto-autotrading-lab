...
- **注文数量が取引所の数量刻み（`sizeStep`）に丸められていません。** GMOコインの BTC は最小注文数量・刻みとも 0.00001 です。現在は小数8桁で切り捨てるだけなので、刻みの倍数にならず注文が拒否されます。
...
> 売り注文は `realTrading.isStopped=true` でも実行します（上記 8.2）。
