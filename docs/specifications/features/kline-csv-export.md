...
| 項目 | 内容 |
| --- | --- |
| 想定読者 | K線CSV作成機能を実装・レビューする開発者 |
| 読んだあとできること | 取得するK線の範囲とCSVの形式を、実装を見ずに説明できる |
| 状態 | 現行 |
| 機密区分 | 公開可 |
| 作成者 | リポジトリ管理者 |
| 保守責任者 | リポジトリ管理者 |
...
GMOコイン Public API から取得したK線データを、CSV形式で保存できるようにします。
...
指定された日付範囲を受け取り、1日ごとに Public API のK線取得を呼び出します。取得したK線データを1つの一覧にまとめ、openTime の昇順に並べ、重複を排除した上でバックテスト入力用のCSVファイルとして保存します。
...
- 期待する結果: 3日分のK線データが取得され、1つのCSVファイルとして保存される。
