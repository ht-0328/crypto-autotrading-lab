...
この文書では、Phase1（最初の段階）で開発するシステムの仕様を定義する。
...
1. 設定ファイル（YAML）を読み込む
2. GMOコイン Public API から最新の価格データを取得する
...
### K線の取得対象日と判定がスキップされる時間帯

K線の取得対象日は、取引所の営業日区切り（朝6時）に合わせて切り替わります（`TradingApplication.resolveKlineTargetDate()`）。6時を過ぎると当日分のK線しか取得しないため、判定に必要な本数が揃うまでは判定を行いません。

- 必要な本数は戦略により異なり、最大で15本（`atr_length` が 14 の場合）です。
- 5分足の場合、**毎日 6:00 から約75分間は「データ不足」として判定がスキップされます。** この間は保有中でも利確・損切りの判定は行われません。
- これは既知の制約です。前日分とあわせて取得する改善は今後の課題として [改善計画のバックログ](../improvements/backlog.md) に登録しています。
