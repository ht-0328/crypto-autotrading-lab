...
- **CSV読み込みの隠蔽**: CSVのパース、ヘッダーの検証、ダブルクォートの処理などは外部ライブラリ（opencsvなど）に依存するため、すべて `infrastructure` 層に配置する。
- **戻り値の型**: アプリケーション層やドメイン層には、ファイル操作に関する情報を一切漏らさず、純粋な `List<Kline>` のみを返す。
