...
- **ドメインロジックの排除**: CSVエクスポート処理には複雑なドメイン知識（売買判定など）がないため、主に `application` と `infrastructure` の協調として実装する。
- **データ型**: GMO Public API から取得した生データ（DTO）は `infrastructure` 内で `domain` の `Kline` モデルに変換し、アプリケーション層では `Kline` モデルのリストを扱う。
- **ファイル保存の抽象化**: CSV保存処理は特定のライブラリ（opencsv等）に依存するため `infrastructure` に配置し、抽象インターフェースを通じて呼び出す。
...
2. `KlineCsvExportApplication` が開始日から終了日までの各日付（yyyyMMdd形式の文字列またはLocalDate）のリストを作る
...
- アプリケーションは `LocalDate` などを用いて `2026-05-01`, `2026-05-02`, `2026-05-03` を生成し、それぞれ `yyyyMMdd` 形式の文字列 `20260501`, `20260502`, `20260503` としてAPIに渡す。
