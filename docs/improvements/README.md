...
作業を依頼するときは、実施したいファイルのパスを指定してください。各ファイルは単体で完結しています。対象の指摘・変更対象・実施手順・受け入れ条件・検証手順が書いてあります。
...
3ツールで同じリポジトリをレビューし、結果を突き合わせて作成しています（2026-08-29）。使ったのは Claude Code / Codex / Antigravity です。
...
| 状態 | ファイル | 内容 | 重要度 |
| --- | --- | --- | --- |
| 実施済み | [PR01](pr01-docker-clean-scope.md) | ホストの Docker を全消しするスクリプトを安全にする | 高 |
| 実施済み | [PR03](pr03-private-api-log-leak.md) | Private API レスポンスのログ流出を止める | 高 |
| 実施済み | [PR02](pr02-cloud-run-config.md) | Cloud Run で設定が効かない・ログが残らない問題を直す | 高 |
| 実施済み | [PR04](pr04-state-repository-crash-safe.md) | 状態保存をクラッシュセーフにする | 高 |
| 実施済み | [PR05](pr05-phase1-real-order-guard.md) | Phase1 で実注文を構造的に不可能にする | 高 |

## 第2波: 仕様・ドキュメント・CI の整合

| 状態 | ファイル | 内容 | 重要度 |
| --- | --- | --- | --- |
| 実施済み | [PR09](pr09-ci-compose-consistency.md) | CI / Compose の整合と安全側固定 | 高 |
| 実施済み | [PR06](pr06-backtest-execution-model.md) | バックテストの約定モデルを是正する | 高 |
| 実施済み | [PR07](pr07-real-order-spec-separation.md) | 実注文機能を Phase1 の仕様から分離する | 高 |
| 実施済み | [PR08](pr08-doc-consistency.md) | 仕様書・設計書の食い違いを解消する | 中 |
| 実施済み | [PR10](pr10-config-fail-fast.md) | 設定の fail-fast と環境変数契約の統一 | 中 |
...
| 状態 | ファイル | 内容 | 重要度 |
| --- | --- | --- | --- |
| 実施済み | [PR11](pr11-zensical-broken-links.md) | 公開サイトのリンク切れ197件を解消し、再発をCIで止める | 高 |
| 実施済み | [PR12](pr12-zensical-readability.md) | 安全上の警告を admonition にし、章タイトルと目次の食い違いを直す | 中 |

**[PR11](pr11-zensical-broken-links.md) を先に実施してください。** 両方とも同じファイルを触るため、順序を逆にすると衝突します。

## 推奨する着手順

**第1波: 実害を止める**

1. [PR01](pr01-docker-clean-scope.md)（実行1回で被害が出るため最優先）
2. [PR03](pr03-private-api-log-leak.md) → [PR02](pr02-cloud-run-config.md)（ログ流出を止めてから標準出力へ流す。順序が逆だと流出が増える）
3. [PR04](pr04-state-repository-crash-safe.md)
4. [PR05](pr05-phase1-real-order-guard.md)

**第2波: 整合を取る**

5. [PR09](pr09-ci-compose-consistency.md)
6. [PR06](pr06-backtest-execution-model.md)
7. [PR07](pr07-real-order-spec-separation.md) → [PR08](pr08-doc-consistency.md) → [PR10](pr10-config-fail-fast.md)

**第4波: ドキュメントサイト**

8. [PR11](pr11-zensical-broken-links.md) → [PR12](pr12-zensical-readability.md)（どちらも実装には影響しないため、第1波・第2波とは独立に進められる）
