...
| U | インフラ | `docker compose up`（README の推奨手順）が、1回実行して終了するバッチに `restart: unless-stopped` を付けているため無限再起動し、設定は本物の GMO Public API を向いている。WireMock は起動するだけで Public API には使われない。`version: "3.8"` は非推奨、`wiremock:latest` は CI の `3.5.2` と非固定 | [local.yml:1-31](https://github.com/ht-0328/crypto-autotrading-lab/blob/main/docker/compose/local.yml) | 高 | [pr09](pr09-ci-compose-consistency.md) |
