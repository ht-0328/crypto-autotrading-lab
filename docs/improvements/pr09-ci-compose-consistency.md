...
| 読んだあとできること | CI と Compose の設定を一致させ、安全側に固定できる |
...
- **U（重要度: 高）**: README が案内する `docker compose ... up --build` が危険です。アプリは1回実行して終了するバッチです。しかし `restart: unless-stopped` が付いており、無限に再起動します。
  設定も本物の GMO Public API を向いている。`API_BASE_URL` は実装が読まないため無視される。同時に起動する WireMock は Public API には使われない。
