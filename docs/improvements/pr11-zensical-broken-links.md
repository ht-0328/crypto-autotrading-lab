...
[Zensical](https://zensical.org/) による [ドキュメントサイト](https://ht-0328.github.io/crypto-autotrading-lab/) を公開しています。これまで問題にならなかったリンクの書き方が、実害を出しています。

Zensical がビルドするのは `docs/` 配下だけです。`docs/` の外を指す相対リンクは、GitHub 上では正しく動きます。**しかし公開サイトでは 404 になります。**
...
絶対 URL は `main` を指します。作業ブランチからは `main` 時点のコードを参照することになります。ドキュメントは「マージ後の状態」を説明するものなので、この挙動で問題ありません。
