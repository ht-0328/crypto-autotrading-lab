...
| 項目 | 内容 |
| --- | --- |
| 想定読者 | リアル取引が停止したときに対応する運用担当者 |
| 読んだあとできること | 停止の理由を特定し、安全に再開できる |
| 状態 | 現行 |
| 機密区分 | 公開可 |
| 作成者 | リポジトリ管理者 |
| 保守責任者 | リポジトリ管理者 |
...
- 15分待てない場合の手順です。**動いている実行がないことを確認してから**ロックを削除します。動いている実行のロックを消すと、2つの実行が同時に注文を出す可能性があります。

## 手順

停止の理由を確認してから、状態を戻します。理由を確認せずに再開しないでください。
