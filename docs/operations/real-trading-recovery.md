...
!!! danger "本番でこの状態が起こりえます"

    2026-08-30 から本番は Phase3 で、実注文が有効です。**この手順は実際に必要になります。** 停止したまま放置すると、保有したポジションを売る判断も行われません。

    `app.phase` が 3 未満の環境、または `real_trade_enabled` が `false` の環境では、起動時ガードにより実注文経路に入らないため、この状態にはなりません。
...
`state.json` の `realTrading.latestOrder` に `orderId` が無く、`status` が `WAITING` のまま停止していることがあります。これは**注文を送信する直前に保存した意図が残ったまま、結果を記録できずに終わった**状態です。

**取引所に注文が届いたかどうかが分かりません。** アプリは新しい注文を出さずに停止します。

### 確認すること

1. GMOコインの取引履歴と注文履歴を開く。
2. `latestOrder.orderedAt` の時刻の前後に、`side` と `requestedSize` に一致する注文があるか確認する。

| 取引所の状態 | 対応 |
| --- | --- |
| 注文が無い | 届いていない。`latestOrder` を消して停止を解除してよい |
| 注文があり約定済み | 届いている。`state.json` の保有状態を取引所の実態に合わせてから停止を解除する |
| 注文があり未約定 | 取引所の画面でキャンセルするか、約定を待つ。約定後に保有状態を合わせる |

**取引所の実態を確認せずに停止を解除しないでください。** 状態が食い違ったまま次の注文に進みます。
