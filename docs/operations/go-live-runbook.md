...
4. 毎日、次を確認する。
   ...
   | 定期実行が止まっていないか | [デプロイ状態の確認 (gcp/09-status-check.md)](gcp/09-status-check.md) |
...
3. **外部からの監視を用意する。** アプリは「自分が動いていないこと」を通知できません。ジョブが起動しなくなったとき、通知は「来ない」だけです。日次サマリーが届かないことに気付ける仕組みが要ります。
   - 止まっているかどうかは [デプロイ状態の確認 (gcp/09-status-check.md)](gcp/09-status-check.md) で判定できます。ただし**これは実行したときにしか動きません。** 誰も実行しなければ止まったことに気付けないため、外部からの監視の代わりにはなりません。
