...
| 項目 | 内容 |
| --- | --- |
| 想定読者 | GitHub Actions の設定をする運用担当者 |
| 読んだあとできること | デプロイに必要な設定値を Variables として登録できる |
| 状態 | 現行 |
| 機密区分 | 公開可 |
| 作成者 | リポジトリ管理者 |
| 保守責任者 | リポジトリ管理者 |
...
GitHub Actions が GCP へデプロイするには、接続先を教える必要があります。「どのプロジェクトの、どの入り口を使うか」です。
...
!!! warning "API_BASE_URL は使われません"
    ...
    `API_BASE_URL` を登録する運用は廃止しています。アプリはこの名前を読みません。`API_PUBLIC_BASE_URL` と `API_PRIVATE_BASE_URL` に登録し直し、古い `API_BASE_URL` は削除してください。
...
!!! warning "秘密情報について"
    ...
    APIキーやシークレットは Variables に登録しません。隣の **Secrets** タブに登録してください。
