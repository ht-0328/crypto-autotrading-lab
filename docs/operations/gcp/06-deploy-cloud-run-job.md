...
!!! danger "デプロイは実資金の動きに影響します"

    2026-08-30 から本番は Phase3 で、実注文が有効です。このワークフローで渡す設定によって、実際の注文が行われるかどうかが変わります。

    実発注されるのは `APP_PHASE` が 3 以上、`REAL_TRADING_ENABLED` が `true`、`REAL_TRADING_DRY_RUN` が `false` の**3つがそろったときだけ**です。値の意味と、未登録のときにどうなるかは [05-github-actions-variables.md](05-github-actions-variables.md) を参照してください。
